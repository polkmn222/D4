...
class SlackMessageProvider(BaseMessageProvider):
    ...
    @staticmethod
    def _absolute_image_url(image_url: str) -> Optional[str]:
    ...
    def send(self, db: Any, payload: MessageDispatchPayload) -> Dict[str, Any]:
        ...
        header = f"{payload.record_type} dev/test dispatch"
        delivery_context = f"Provider: `slack`\nContact: `{payload.contact_id}`\nTemplate: `{payload.template_id or 'custom'}`"
                ...
                blocks.append(
                    {
                        ...
                        "text": {
                            ...
                            "text": (
                                "*Image*\n"
                                "Preview skipped because `APP_BASE_URL` is not configured. "
                                "The MMS request still includes the uploaded attachment in D4."
                            ),
                        },
                    }
                )
        ...
        blocks.append(
            {
                ...
                "elements": [
                    {
                        "type": "mrkdwn",
                        "text": "This message came from the D4 dev/test Slack provider. No carrier SMS/LMS/MMS delivery was attempted.",
                    }
                ],
            }
        )
