...
async def list_tasks(request: Request, db: Session = Depends(get_db)):
    ...
    items = []
    for t in tasks:
        contact = ContactService.get_contact(db, t.contact) if t.contact else None
        items.append({
            "id": t.id,
    ...
    columns = ["name", "status", "priority", "contact"]
...
async def task_detail(request: Request, task_id: str, db: Session = Depends(get_db)):
    try:
        ...
        details = {
            ...
            "Contact": f"{contact.first_name} {contact.last_name}" if contact else "",
            "Contact_Hidden_Ref": task.contact if task.contact else "",
            "Message": task.message if task.message else "",
            "Message_Hidden_Ref": task.message if task.message else "",
        ...
        logger.error(f"Error loading task detail: {e}")
        return RedirectResponse(url="/tasks?error=An+unexpected+error+occurred.+Please+try+again")
