...
async def vehicle_spec_detail(request: Request, spec_id: str, db: Session = Depends(get_db)):
    try:
        ...
        logger.error(f"Error loading brand detail: {e}")
        return RedirectResponse(url="/vehicle_specifications?error=An+unexpected+error+occurred.+Please+try+again")
...
async def create_spec(
    name: str = Form(...),
    record_type: str = Form(RecordType.BRAND),
