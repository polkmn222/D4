...
async def lead_detail(request: Request, lead_id: str, db: Session = Depends(get_db)):
    try:
        ...
        return RedirectResponse(url="/leads?error=An+unexpected+error+occurred.+Please+try+again")

@router.get("/leads")
async def list_leads(request: Request, db: Session = Depends(get_db)):
    # Removed try-except block as BaseService.list handles exceptions by returning an empty list,
    # and LeadService.get_leads now reflects this. An empty list will be rendered instead of redirecting.
    ...
    for l in leads:
        ...
        items.append({
            ...
            "model": model.name if model else "",
            "status": l.status.value if hasattr(l.status, "value") else (l.status if l.status else ""),
...
async def update_lead(
    ...
    try:
        ...
        return RedirectResponse(url=f"/leads/{lead_id}?error=Error+updating+lead")
