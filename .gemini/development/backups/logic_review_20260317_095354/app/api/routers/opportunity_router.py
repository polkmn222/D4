...
from ...core.enums import OpportunityStage, OpportunityStatus
from ... import models
...
async def opportunity_detail(request: Request, opp_id: str, db: Session = Depends(get_db)):
    try:
        ...
        account = AccountService.get_account(db, opp.account) if opp.account else None
        ...
        details = {
            ...
            "Account_Hidden_Ref": opp.account,
...
async def update_opportunity(
    ...
    name: str = Form(...),
    amount: int = Form(0),
    stage: str = Form(OpportunityStage.PROSPECTING.value),
    status: str = Form(OpportunityStatus.OPEN.value),
    ...
    OpportunityService.update_opportunity(
        db, opp_id, account=account, name=name, amount=amount, 
...
async def create_opportunity(
    ...
    try:
        opp = OpportunityService.create_opportunity(
            db, account=account, name=name, amount=amount, 
            stage=stage, status=status, probability=probability,
            brand=brand, model=model,
            asset=asset, product=product
        )
