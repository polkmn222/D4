...
async def contact_detail(request: Request, contact_id: str, db: Session = Depends(get_db)):
    try:
        ...
        account = AccountService.get_account(db, contact.account) if contact.account else None
        details = {
            "Name": f"{contact.first_name if contact.first_name else ''} {contact.last_name if contact.last_name else ''}".strip() or "",
            "Email": contact.email if contact.email else "",
            "Phone": contact.phone if contact.phone else "",
            "Account": account.name if account else "",
            "Account_Hidden_Ref": contact.account if contact.account else "",
            "Created Date": contact.created_at.strftime("%Y-%m-%d %H:%M") if contact.created_at else ""
        }
        
        # Related Assets
        assets = AssetService.get_assets(db, account=contact.account)
        related_lists = []
        if assets:
