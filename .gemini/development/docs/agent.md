...
- Regardless of the magnitude of change, store a full pre-change codebase backup (excluding `.git/`, `.venv/`, `node_modules/`, and the `backups/` folder itself) under the matching grouped range folder, for example `.gemini/development/backups/101_199/phase148/`, using phase-specific folders and relative paths.
...
- Use `.gemini/development/.pytest_cache` as the canonical pytest cache location.
