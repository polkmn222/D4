...
function initLookup(containerId, objectType, fieldName, initialValueId = null, initialValueName = null) {
    ...
    const selectItem = (item) => {
        ...
        const event = new CustomEvent('lookupSelected', { detail: { item, fieldName } });
    ...
    function getIconText(type) {
        const texts = {
            'Lead': 'LQ', 'Account': 'A', 'Contact': 'C', 'Opportunity': 'O',
            'Asset': 'AS', 'Product': 'P', 'Brand': 'B', 'Campaign': 'CP', 'Model': 'M'
        };
        return texts[type] || 'R';
    }
