...
class AssetService(BaseService[Asset]):
    ...
    @classmethod
    def create_asset(cls, db: Session, name: str, account_id: Optional[str] = None, product_id: Optional[str] = None, **kwargs) -> Asset:
        return cls.create(db, name=name, account_id=account_id, product_id=product_id, **kwargs)

    @classmethod
    def get_assets(cls, db: Session, account_id: Optional[str] = None) -> List[Asset]:
