...
async def asset_detail(request: Request, asset_id: str, db: Session = Depends(get_db)):
    try:
        ...
        brand = BrandService.get_vehicle_spec(db, model.brand_id) if (model and model.brand_id) else None
        ...
        details = {
            ...
            "Account_Hidden_Ref": asset.account_id,
            "Brand": brand.name if brand else None,
            "Brand_Hidden_Ref": brand.id if brand else None,
            "Model": model.name if model else None,
            "Model_Hidden_Ref": asset.model_id,
            "Product": prod.name if prod else None,
            "Product_Hidden_Ref": asset.product_id
        }
        return templates.TemplateResponse("detail_view.html", {
            "request": request, "object_type": "Asset", "plural_type": "assets",
            "record_id": asset_id, "record_name": asset.name if asset.name else "N/A",
...
async def update_asset_endpoint(
    ...
    try:
        AssetService.update_asset(
            db, asset_id, name=name, account_id=account_id, product_id=product_id, 
...
async def create_asset_endpoint(
    ...
    try:
        asset = AssetService.create_asset(
            db, name=name, account_id=account_id, product_id=product_id, 
            brand_id=brand_id, model_id=model_id, vin=vin, price=price, status=status
        )
