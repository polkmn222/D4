...
async def inline_save(object_type: str, record_id: str, field: str = Form(...), value: str = Form(...), db: Session = Depends(get_db)):
    ...
    if record:
        ...
        if field_norm == "account_hidden_ref": update_data = {"account_id": value}
        elif field_norm == "brand_hidden_ref": update_data = {"brand_id": value}
        elif field_norm == "model_hidden_ref": update_data = {"model_id": value}
        elif field_norm == "product_hidden_ref": update_data = {"product_id": value}
        elif field_norm == "asset_hidden_ref": update_data = {"asset_id": value}
