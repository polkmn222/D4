...
async def contact_detail(request: Request, contact_id: str, db: Session = Depends(get_db)):
    try:
        ...
        account = AccountService.get_account(db, contact.account_id) if contact.account_id else None
        details = {
            "Name": f"{contact.first_name if contact.first_name else ''} {contact.last_name if contact.last_name else ''}".strip() or None,
            "Email": contact.email,
            "Phone": contact.phone,
            "Account": account.name if account else None,
            "Account_Hidden_Ref": contact.account_id,
            "Created Date": contact.created_at.strftime("%Y-%m-%d %H:%M") if contact.created_at else None
        }
...
async def delete_contact(request: Request, contact_id: str, db: Session = Depends(get_db)):
    ...
    return RedirectResponse(url="/contacts?success=Record+deleted+successfully", status_code=303)
