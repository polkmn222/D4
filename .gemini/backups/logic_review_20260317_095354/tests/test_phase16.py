...
def test_lead_atomic_operations(db):
    ...
    assert lead.is_flow_enabled is False
    ...
    # Test toggle_flow
    LeadService.toggle_flow(db, lead.id, True)
    db.refresh(lead)
    assert lead.is_flow_enabled is True
    
    LeadService.toggle_flow(db, lead.id, False)
    db.refresh(lead)
    assert lead.is_flow_enabled is False
...
def test_opportunity_atomic_operations(db):
    ...
    assert opp.is_flow_enabled is False
    ...
    # Test toggle_flow
    OpportunityService.toggle_flow(db, opp.id, True)
    db.refresh(opp)
    assert opp.is_flow_enabled is True
