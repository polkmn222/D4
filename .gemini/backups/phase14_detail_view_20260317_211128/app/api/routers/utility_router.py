...
async def lookup_search(
    ...
    if type == "Account" or type == "Contact":
        data = db.query(models.Contact).filter(
            func.or_(
                func.concat(models.Contact.first_name, " ", models.Contact.last_name).ilike(f"%{q}%"),
        ...
        data = db.query(models.Message).filter(models.Message.content.ilike(f"%{q}%")).limit(limit).all()
        for item in data: 
            display = item.content[:50] + "..." if len(item.content) > 50 else item.content
            results.append({"id": item.id, "name": display, "type": "Message"})
