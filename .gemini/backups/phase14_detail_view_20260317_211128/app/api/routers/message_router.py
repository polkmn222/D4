...
async def message_detail(request: Request, message_id: str, db: Session = Depends(get_db)):
    try:
        ...
        details = {
            "Direction": msg.direction if msg.direction else "",
            "Status": msg.status if msg.status else "",
            "Contact": f"{contact.first_name} {contact.last_name}" if contact else "",
            "Contact_Hidden_Ref": msg.contact if msg.contact else "",
            "Template": template.name if template else "",
            "Template_Hidden_Ref": msg.template if msg.template else "",
            "Content": msg.content if msg.content else ""
        }
        
        return templates.TemplateResponse("messages/detail_view.html", {
            "request": request, 
            "object_type": "Message", 
            "plural_type": "messages",
            "record_id": message_id, 
            "record_name": "Message Detail", 
            "details": details,
        ...
        return RedirectResponse(url=f"/messages?error=Error+loading+message+detail:+{str(e).replace(' ', '+')}")
...
async def create_template_route(
    ...
    t = MessageTemplateService.create_template(db, name=name, subject=subject, content=content, record_type=record_type)
...
async def update_template_route(
    ...
    MessageTemplateService.update_template(db, template_id, name=name, subject=subject, content=content, record_type=record_type)
