...
class OpportunityService(BaseService[Opportunity]):
    ...
    @classmethod
    def create_opportunity(cls, db: Session, account: str = None, name: str = "", amount: int = 0, stage: str = "Prospecting", status: str = "Open", probability: int = 10, **kwargs) -> Opportunity:
        return cls.create(db, account=account, name=name, amount=amount, stage=stage, status=status, probability=probability, **kwargs)

    @classmethod
    def get_by_account(cls, db: Session, account: str) -> List[Opportunity]:
