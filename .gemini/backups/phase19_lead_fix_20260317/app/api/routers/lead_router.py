...
async def lead_detail(request: Request, lead_id: str, db: Session = Depends(get_db)):
    try:
        ...
        details = {
            "Name": f"{lead.first_name if lead.first_name else ''} {lead.last_name if lead.last_name else ''}".strip() or None,
            "Status": lead.status,
            "Brand": brand.name if brand else None,
            "Brand_Hidden_Ref": lead.brand_id,
            "Model": model.name if model else None,
            "Model_Hidden_Ref": lead.model_id,
            "Product": prod.name if prod else None,
            "Product_Hidden_Ref": lead.product_id,
            "Email": lead.email,
            "Phone": lead.phone,
            "Is Followed": "Yes" if lead.is_followed else "No",
            "Created Date": lead.created_at.strftime("%Y-%m-%d %H:%M") if lead.created_at else None
        }
...
async def list_leads(request: Request, db: Session = Depends(get_db)):
    ...
    items = []
    for l in leads:
        model = ModelService.get_model(db, l.model_id) if l.model_id else None
        items.append({
            "id": l.id,
...
async def create_lead(
    ...
    lead = LeadService.create_lead(
        ...
        product_id=product_id, description=description, status="New"
    )
...
async def update_lead(
    ...
    LeadService.update_lead(
        ...
        email=email, phone=phone, status=status, campaign_id=campaign_id,
        brand_id=brand_id, model_id=model_id, 
...
async def convert_lead(
    ...
    account_record_type: str = Form("Individual"),
