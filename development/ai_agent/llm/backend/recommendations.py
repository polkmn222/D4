...
class AIRecommendationService:
    ...
    @classmethod
    def _ensure_opportunity_updated_by_column(cls, db: Session) -> None:
        ...
        with db.bind.begin() as connection:
            connection.execute(text("ALTER TABLE opportunities ADD COLUMN updated_by VARCHAR"))

    @classmethod
    def _already_refreshed_today(cls, db: Session, reference_date: datetime) -> bool:
        """
        Check if any opportunity has been refreshed by 'AI Recommend' today.
        This provides a global throttle for daily temperature refreshes.
        """
        refreshed = db.query(Opportunity).filter(
            Opportunity.deleted_at == None,
            Opportunity.updated_by == cls.AI_AGENT_USER,
            Opportunity.updated_at != None,
        ).first()
        if refreshed:
            updated_at = getattr(refreshed, "updated_at", None)
            updated_kst = make_naive_kst(updated_at) if updated_at else None
            if updated_kst and updated_kst.date() == reference_date.date():
                return True
        return False

    @classmethod
    def refresh_opportunity_temperatures(cls, db: Session) -> None:
        cls._ensure_opportunity_updated_by_column(db)
        ...
        for opp in opportunities:
            ...
            # If last modified date is today AND last modified by is ai agent, do not update.
            rec_updated_at = getattr(opp, "updated_at", None)
            rec_updated_kst = make_naive_kst(rec_updated_at) if rec_updated_at else None
            rec_updated_by = getattr(opp, "updated_by", None)
            
            if rec_updated_kst and rec_updated_kst.date() == reference_date.date() and rec_updated_by == cls.AI_AGENT_USER:
                continue
            ...
            opp.updated_by = cls.AI_AGENT_USER
            opp.updated_at = reference_date
            updated_any = True
    ...
    def get_ai_recommendations(cls, db: Session, limit: int = 10, mode: Optional[str] = None) -> List[Opportunity]:
        ...
        Consistent across Home, Messaging, and AI Agent.
        """
        try:
            ...
            if target_mode == "Hot Deals":
                ...
                for o in all_opps:
                    ...
                    close_date_kst = make_naive_kst(close_date) if close_date else None
                    closed_source = close_date if close_date_kst and close_date_kst <= reference_date else getattr(o, "created_at", None)
                ...
                all_opps = query.filter(~Opportunity.stage.in_(["Closed Won", "Closed Lost"]))\
