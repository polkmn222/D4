...
async def opportunity_detail(request: Request, opp_id: str, db: Session = Depends(get_db)):
    try:
        ...
        lead = LeadService.get_lead(db, lead_id) if lead_id else None
        ...
        if lead:
            related_lists.append({
                ...
                "icon": "lead",
                "view_all_url": f"/leads?id={lead.id}&related=1&back_to=/opportunities/{opp_id}?tab=related",
                "total_count": 1,
                "items": [{
