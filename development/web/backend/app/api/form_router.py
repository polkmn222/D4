...
async def new_lead_modal(
    ...
    try:
        ...
        elif any([brand, model, product]):
            brand_obj = BrandService.get_vehicle_spec(db, brand) if brand else None
            model_obj = ModelService.get_model(db, model) if model else None
            prod_obj = ProductService.get_product(db, product) if product else None
            if model_obj and not brand and getattr(model_obj, "brand", None):
                brand = model_obj.brand
                brand_obj = BrandService.get_vehicle_spec(db, brand)
            initial_values = {
                "brand": brand or "",
                ...
                "product": product or "",
                "product_name": prod_obj.name if prod_obj else "",
            }
        return templates.TemplateResponse(request, "templates/sf_form_modal.html", {
...
async def new_opportunity_modal_base(request: Request, id: str = None, contact: str = None, product: str = None, asset: str = None, brand: str = None, model: str = None, db: Session = Depends(get_db)):
    try:
        return await new_opportunity_modal(request, id, contact, product, asset, brand, model, db)
    except Exception as e:
    ...
    try:
        ...
        if id:
            ...
            if opp:
                ...
                initial_values = {
                    "id": opp.id, "contact": opp.contact, "contact_name": _contact_display_name(contact),
        ...
        elif any([contact, product, asset, brand, model]):
            contact_obj = ContactService.get_contact(db, contact) if contact else None
            brand_obj = VehicleSpecificationService.get_vehicle_spec(db, brand) if brand else None
            model_obj = ModelService.get_model(db, model) if model else None
            prod_obj = ProductService.get_product(db, product) if product else None
            asset_obj = AssetService.get_asset(db, asset) if asset else None
            initial_values = {
                "contact": contact or "",
                "contact_name": _contact_display_name(contact_obj),
                "brand": brand or "",
                "brand_name": brand_obj.name if brand_obj else "",
                "model": model or "",
                "model_name": model_obj.name if model_obj else "",
...
async def new_model_modal_base(request: Request, id: str = None, brand: str = None, db: Session = Depends(get_db)):
    try:
        return await new_model_modal(request, id, brand, db)
    except Exception as e:
...
async def new_model_modal(request: Request, id: str = None, brand: str = None, db: Session = Depends(get_db)):
    try:
        ...
        if id:
            ...
            if model:
                brand_spec = BrandService.get_vehicle_spec(db, model.brand) if model.brand else None
                initial_values = {
                    "id": model.id, "name": model.name, "description": model.description,
                    "brand": model.brand, "brand_name": brand_spec.name if brand_spec else ""
                }
        elif brand:
            brand_spec = BrandService.get_vehicle_spec(db, brand)
            initial_values = {
                "brand": brand,
                "brand_name": brand_spec.name if brand_spec else "",
            }
        return templates.TemplateResponse(request, "templates/sf_form_modal.html", {
...
async def new_product_modal_base(request: Request, id: str = None, brand: str = None, model: str = None, db: Session = Depends(get_db)):
    try:
        return await new_product_modal(request, id, brand, model, db)
    except Exception as e:
        logger.error(f"Error in product modal base: {e}")
        return RedirectResponse(url="/products?error=Error+opening+modal")

@router.get("/products/new")
async def new_product_modal(request: Request, id: str = None, brand: str = None, model: str = None, db: Session = Depends(get_db)):
    try:
        ...
        if id:
            ...
            if prod:
                brand_spec = BrandService.get_vehicle_spec(db, prod.brand) if prod.brand else None
                model = ModelService.get_model(db, prod.model) if prod.model else None
                initial_values = {
                    "id": prod.id, "name": prod.name, 
                    "brand": prod.brand, "brand_name": brand_spec.name if brand_spec else "",
            ...
            if model_obj and not brand and getattr(model_obj, "brand", None):
                brand = model_obj.brand
            brand_spec = BrandService.get_vehicle_spec(db, brand) if brand else None
            initial_values = {
                "brand": brand or "",
                "brand_name": brand_spec.name if brand_spec else "",
                "model": model or "",
...
async def new_asset_modal_base(request: Request, id: str = None, brand: str = None, model: str = None, db: Session = Depends(get_db)):
    try:
        return await new_asset_modal(request, id, brand, model, db)
    except Exception as e:
    ...
    try:
        ...
        if id:
            ...
            if asset:
                ...
                brand_spec = BrandService.get_vehicle_spec(db, asset.brand) if asset.brand else None
                model = ModelService.get_model(db, asset.model) if asset.model else None
                initial_values = {
                    "id": asset.id, "contact": asset.contact, "contact_name": _contact_display_name(contact),
                    "product": asset.product, "product_name": prod.name if prod else "",
                    "brand": asset.brand, "brand_name": brand_spec.name if brand_spec else "",
...
async def new_message_modal_base(request: Request, id: str = None, contact: str = None, template: str = None, db: Session = Depends(get_db)):
    try:
        return await new_message_modal(request, id, contact, template, db)
    except Exception as e:
...
async def new_message_modal(request: Request, id: str = None, contact: str = None, template: str = None, db: Session = Depends(get_db)):
    try:
        fields = ["contact", "record_type", "subject", "content", "status"]
        initial_values = None
        if id:
            ...
            contact_obj = ContactService.get_contact(db, contact) if contact else None
            template_obj = MessageTemplateService.get_template(db, template) if template else None
            initial_values = {
                "contact": contact or "",
                "contact_name": f"{contact_obj.first_name} {contact_obj.last_name}".strip() if contact_obj else "",
                "template": template or "",
                "template_name": template_obj.name if template_obj else "",
            }
        return templates.TemplateResponse(request, "templates/sf_form_modal.html", {
...
async def new_lead_modal_base(
    ...
    try:
        return await new_lead_modal(request, id, brand, model, product, embedded, db)
    except Exception as e:
        logger.error(f"Error in lead modal base: {e}")
        return RedirectResponse(url="/leads?error=Error+opening+modal")

@router.get("/leads/new")
async def new_lead_modal_direct(
    request: Request,
    id: str = None,
    brand: str = None,
    model: str = None,
...
@router.get("/leads/embedded-form")
async def lead_embedded_form_page(
    request: Request,
    id: str = None,
    brand: str = None,
    model: str = None,
    ...
    try:
        ...
        if id:
            lead = LeadService.get_lead(db, id)
            if lead:
                brand_obj = BrandService.get_vehicle_spec(db, lead.brand) if lead.brand else None
                model_obj = ModelService.get_model(db, lead.model) if lead.model else None
                prod_obj = ProductService.get_product(db, lead.product) if lead.product else None
                initial_values = {
                    "id": lead.id, "first_name": lead.first_name, "last_name": lead.last_name,
                    "email": lead.email, "phone": lead.phone, "status": lead.status,
                    "gender": lead.gender, "brand": lead.brand, "brand_name": brand_obj.name if brand_obj else "",
                    "model": lead.model, "model_name": model_obj.name if model_obj else "",
                    "product": lead.product, "product_name": prod_obj.name if prod_obj else "",
                    "description": lead.description,
                }
        elif any([brand, model, product]):
            brand_obj = BrandService.get_vehicle_spec(db, brand) if brand else None
        ...
        return templates.TemplateResponse(request, "leads/embedded_form_page.html", {
            "request": request,
            "object_type": "Lead",
            "plural_type": "leads",
            "brands": brands,
        ...
        logger.error(f"Error loading embedded lead form page: {e}")
        return RedirectResponse(url="/leads?error=Error+opening+embedded+form")
...
async def new_template_modal(request: Request, id: str = None, db: Session = Depends(get_db)):
    try:
        fields = ["name", "record_type", "subject", "image", "content"]
        initial_values = None
        if id:
