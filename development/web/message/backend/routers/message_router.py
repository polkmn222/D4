...
async def create_message_route(
    ...
    return RedirectResponse(url=f"/messages/{message.id}?success=Record+created+successfully", status_code=303)
...
async def update_message_route(
    ...
    MessageService.update_message(
        ...
        content=content,
        status=status,
        _force_null_fields=[field for field, value in {"contact": contact, "template": template}.items() if value in (None, "")],
    )
...
async def message_detail(request: Request, message_id: str, db: Session = Depends(get_db)):
    ...
    if contact:
        related_lists.append({
            ...
            "view_all_url": f"/contacts?id={contact.id}&related=1&back_to=/messages/{message_id}?tab=related",
            "total_count": 1,
            "items": [{
    ...
    if template:
        related_lists.append({
            "title": "Template",
            "icon": "template",
            "columns": ["Name", "Type", "Subject"],
            "view_all_url": f"/message_templates?id={template.id}&related=1&back_to=/messages/{message_id}?tab=related",
            "total_count": 1,
            "items": [{
                "name": template.name or "",
                "type": template.record_type or "",
                "subject": template.subject or "",
                "_href": f"/message_templates/{template.id}",
            }],
        })
