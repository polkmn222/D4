...
from db.models import Contact, MessageSend
...
def _clear_template_image_fields(template) -> None:
    template.image_url = ""
    template.file_path = ""
...
async def list_templates(request: Request, db: Session = Depends(get_db)):
    try:
        ...
        back_url = request.query_params.get("back_to") or "/message_templates"
        record_id = request.query_params.get("id")
        templates_data = MessageTemplateService.get_templates(db)
        if record_id:
            templates_data = [template for template in templates_data if template.id == record_id]
            related_mode = True
        items = []
        for t in templates_data:
            items.append({
                ...
                "_href": f"/message_templates/{t.id}",
            })
        if related_mode:
            return templates.TemplateResponse(request, "related/list_view.html", {
                ...
                "back_url": back_url,
                "new_url": "/message_templates/new-modal",
                "page_icon": "template",
                "show_actions": True,
            })
...
async def template_detail(request: Request, template_id: str, db: Session = Depends(get_db)):
    try:
        ...
        related_lists = []
        template_messages = (
            db.query(MessageSend)
            .filter(MessageSend.template == template_id, MessageSend.deleted_at == None)
            .order_by(MessageSend.created_at.desc())
            .all()
        )
        if template_messages:
            contact_ids = {message.contact for message in template_messages if message.contact}
            contact_map = {}
            if contact_ids:
            ...
            related_lists.append({
                ...
                "new_url": f"/messages/new-modal?template={template_id}",
                "view_all_url": f"/messages?template={template_id}&related=1&back_to=/message_templates/{template_id}?tab=related",
                "total_count": len(template_messages),
                "items": [{
                    "name": (f"{contact_map[msg.contact].first_name if msg.contact and contact_map.get(msg.contact) and contact_map[msg.contact].first_name else ''} {contact_map[msg.contact].last_name if msg.contact and contact_map.get(msg.contact) and contact_map[msg.contact].last_name else ''}".strip() if msg.contact and contact_map.get(msg.contact) else "Unknown") or "Unknown",
                    "direction": msg.direction or "",
                    "status": msg.status or "",
                    "_href": f"/messages/{msg.id}",
                } for msg in template_messages[:5]],
            })
...
async def create_template_route(
    ...
    try:
        ...
        if normalized_record_type == "MMS" and (not image or not image.filename):
            return RedirectResponse(
                url="/message_templates?error=MMS+templates+require+a+JPG+image+under+500KB",
                status_code=303,
            )
        ...
        update_data = {"name": name, "subject": subject, "content": content_val, "record_type": normalized_record_type}
        # Filter out None and Form objects
        ...
        if image and image.filename:
            ...
            if file_content:
                ...
                t.file_path = stored_image.file_path
                t.image_url = stored_image.file_path
                t.attachment_id = attachment.id
                db.commit()
...
async def update_template_route(
    ...
    try:
        t = MessageTemplateService.get_template(db, template_id)
        if not t:
            return RedirectResponse(url="/message_templates?error=Template+not+found")
        ...
        if image and image.filename:
            ...
            if file_content:
                ...
                if getattr(t, "attachment_id", None) or getattr(t, "image_url", None):
                    _remove_template_image(db, t)
...
@handle_agent_errors
async def template_upload(request: Request, template_id: str, file: UploadFile = File(...), db: Session = Depends(get_db)):
    try:
        ...
        return {
            "status": "success",
            "image_url": stored_image.file_path,
...
@handle_agent_errors
async def clear_template_image(request: Request, template_id: str, db: Session = Depends(get_db)):
    try:
        template = MessageTemplateService.get_template(db, template_id)
        if not template:
            return JSONResponse(status_code=404, content={"status": "error", "message": "Template not found"})
        _remove_template_image(db, template)
        _clear_template_image_fields(template)
        db.commit()
        db.refresh(template)
