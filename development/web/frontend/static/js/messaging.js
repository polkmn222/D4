...
window.Messaging = (function() {
    ...
    let modalPendingTemplateImageFile = null;
    let modalPendingTemplateImagePreviewUrl = null;
    ...
    const TemplateManager = {
        resetModalPendingImageState() {
            modalPendingTemplateImageFile = null;
            if (modalPendingTemplateImagePreviewUrl) {
                URL.revokeObjectURL(modalPendingTemplateImagePreviewUrl);
                modalPendingTemplateImagePreviewUrl = null;
            }
        },

        async uploadPendingModalImage() {
            if (!modalPendingTemplateImageFile) return null;

            const formData = new FormData();
            formData.append('file', modalPendingTemplateImageFile);
            ...
            this.resetModalPendingImageState();
            return result;
        },
        ...
        clearModalFile() {
            ...
            document.getElementById('modal-tpl-preview').src = '';
            document.getElementById('modal-tpl-file-name').innerText = '';
            document.getElementById('modal-tpl-file-info').innerText = '';
            const imageInput = document.getElementById('modal-tpl-image');
        ...
        async handleModalUpload(input) {
            ...
            this.resetModalPendingImageState();
            modalPendingTemplateImageFile = file;
            modalPendingTemplateImagePreviewUrl = URL.createObjectURL(file);
            document.getElementById('modal-tpl-preview').src = modalPendingTemplateImagePreviewUrl;
            document.getElementById('modal-tpl-file-name').innerText = file.name;
            document.getElementById('modal-tpl-file-info').innerText = `${(file.size / 1024).toFixed(1)} KB`;
            document.getElementById('modal-image-preview-container').style.display = 'block';
            showToast("Image selected. It will be saved when you save the template.");
        },
        ...
        async save() {
            ...
            try {
                if (record_type === 'MMS' && modalPendingTemplateImageFile) {
                    await this.uploadPendingModalImage();
                }
