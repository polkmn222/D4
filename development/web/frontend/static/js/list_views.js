...
function initializeObjectListView(config) {
    ...
    function renderFilterConditions(conditions) {
        ...
        setupFilterList.innerHTML = normalizedConditions.map((condition) => `
            <div class="sf-list-view-filter-row">
                ...
                <button type="button" class="sf-list-view-remove-filter" onclick="event.stopPropagation(); window.${removeFilterFunctionName}(this)">Remove</button>
            </div>
        `).join("");
    }
    ...
    function updateEmptyState() {
        ...
        if (tableWrapper) {
            tableWrapper.style.display = shouldShowEmpty ? "none" : "block";
        } else {
    ...
    function applyRowWindowing() {
        ...
        let visibleCount = 0;
        allRows.forEach((row) => {
            if (row.dataset.filterVisible === "true") {
                if (visibleCount < state.rowDisplayLimit) {
                    row.style.display = "";
                    visibleCount++;
                } else {
        ...
        const filteredRowsCount = getFilteredRows().length;
        if (listMoreLoader) {
            listMoreLoader.style.display = filteredRowsCount > state.rowDisplayLimit || isLoadingMoreRows ? "flex" : "none";
    ...
    function renderView() {
        ...
        // Perceived performance: Show skeleton if switching to a view that might be empty or loading
        if (listSkeleton && tableWrapper) {
            tableWrapper.style.display = "none";
            emptyState.style.display = "none";
            listSkeleton.style.display = "block";
            
            // Brief delay to simulate loading and show the skeleton
            setTimeout(() => {
