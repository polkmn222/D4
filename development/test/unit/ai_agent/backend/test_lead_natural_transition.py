...
class TestLeadCreateNaturalTransition:
    ...
    def test_lead_create_sets_conversation_context(self):
        ...
        with patch("web.backend.app.services.lead_service.LeadService.create_lead", return_value=mock_lead), \
             ...
             patch("web.backend.app.services.model_service.ModelService.get_model", return_value=None), \
             patch("web.backend.app.services.product_service.ProductService.get_product", return_value=None), \
             patch("ai_agent.llm.backend.conversation_context.ConversationContextStore.clear_pending_create") as mock_clear, \
             patch("ai_agent.llm.backend.conversation_context.ConversationContextStore.remember_created") as mock_created, \
             patch("ai_agent.llm.backend.conversation_context.ConversationContextStore.remember_object") as mock_obj:
...
class TestLeadOpenRecordContract:
    def _run(self, coro):
        import asyncio
        return asyncio.get_event_loop().run_until_complete(coro)

    def test_lead_manage_returns_open_record_contract(self):
        from ai_agent.ui.backend.service import AiAgentService

        lead = MockLead(record_id="LEAD_MGMT_195", first_name="Atomic", last_name="Lead")
        db = MagicMock()
        agent_output = {
            "intent": "MANAGE",
            "object_type": "lead",
            "record_id": "LEAD_MGMT_195",
            "language_preference": "eng",
            "score": 1.0,
        }
        ...
        with patch("web.backend.app.services.lead_service.LeadService.get_lead", return_value=lead), \
             ...
             patch("web.backend.app.services.product_service.ProductService.get_product", return_value=None), \
             patch("ai_agent.llm.backend.conversation_context.ConversationContextStore.remember_object"):
            result = self._run(AiAgentService._execute_intent(db, agent_output, "manage lead LEAD_MGMT_195"))

        assert result["intent"] == "OPEN_RECORD"
        assert result["record_id"] == "LEAD_MGMT_195"
        assert result["redirect_url"] == "/leads/LEAD_MGMT_195"
        assert result["chat_card"]["mode"] == "view"
