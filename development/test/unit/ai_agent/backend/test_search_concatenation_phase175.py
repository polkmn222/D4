...
def test_search_fields_concatenation_lead():
    ...
    assert "TRIM(CONCAT_WS(' ', l.first_name, l.last_name)) ILIKE '%Test User%'" in search_sql["where"]
    ...
    assert "COALESCE(m.name, l.model) ILIKE '%Test User%'" in search_sql["where"]
...
def test_search_fields_concatenation_contact():
    ...
    assert "TRIM(CONCAT_WS(' ', c.first_name, c.last_name)) ILIKE '%John Doe%'" in search_sql["where"]
    assert "c.first_name ILIKE '%John Doe%'" in search_sql["where"]
