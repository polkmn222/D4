...
def test_default_query_parts_consistency():
    ...
    assert "AS model" in parts["select"]
    assert "LEFT JOIN models" in parts["from"]

def test_normalize_lead_lookup_inputs_resolves_names_to_ids():
    db = MagicMock()
    query = MagicMock()
    db.query.return_value = query
    query.filter.return_value.first.side_effect = [
        MockLookup("BRAND_1", "Genesis"),
        MockLookup("MODEL_1", "GV80"),
        MockLookup("PRODUCT_1", "GV80 AWD"),
    ]

    normalized = AiAgentService._normalize_lead_lookup_inputs(db, {
        "brand": "Genesis",
        "model": "GV80",
        "product": "GV80 AWD",
    })

    assert normalized["brand"] == "BRAND_1"
    assert normalized["model"] == "MODEL_1"
    assert normalized["product"] == "PRODUCT_1"
