...
CSS_PATH = os.path.join(
    ...
    "ai_agent", "ui", "frontend", "static", "css", "ai_agent.css"
)

JS_PATH = os.path.join(
    os.path.dirname(__file__), "..", "..", "..", "..",
    "ai_agent", "ui", "frontend", "static", "js", "ai_agent.js"
)
...
class TestDeleteConfirmationShortcut:
    """Verify pending delete confirmation is handled before another chat round-trip."""

    def test_js_uses_delete_shortcut_helper(self):
        js = _read(JS_PATH)
        assert "function consumePendingDeleteShortcut" in js
        assert "sendAiMessage(deleteShortcut.actualQuery)" in js

    def test_append_chat_message_no_longer_executes_pending_delete(self):
        js = _read(JS_PATH)
        assert "Handle [Yes] / [Cancel] for pending delete confirmation" not in js
