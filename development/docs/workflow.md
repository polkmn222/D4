...
- Review all active markdown files under `development/docs/` before making project changes that affect behavior, architecture, workflow, deployment, or standards.
- Use this review priority when the docs set is large: `docs/agent.md` -> `docs/codex-working-rules.md` -> `docs/testing.md` -> other task-relevant `docs/*.md` -> `docs/testing/*.md` -> D4-specific skill guides -> imported agency skill docs.
- When working on tests, also review the canonical testing docs under `development/docs/testing/`.
- Before planning or running unit tests, read the canonical testing docs under `development/docs/testing/`.
- Create new project-level markdown files in `development/docs/` unless they are phase tracking artifacts in `task/`, `Implementation/`, or `Walkthrough/`, or a folder-local README.

### 2. Backup

- Back up STRICTLY the modified modules (folders) into `backups/<range>/phaseN/` before starting execution.
...
- For code changes, this targeted, module-specific backup is mandatory.
...
- Keep pytest cache output consolidated at `development/.pytest_cache` only.
...
- If a secondary `.pytest_cache` appears at the repository root or in another subdirectory, treat it as accidental local state and merge it back into `development/.pytest_cache`.
...
3. **`.codex` Scope Limit**: The AI must not exhaustively read the generic skill library under `development/.codex/`. It may read only project-relevant metadata, local config, local instructions, and files directly referenced by active project docs.
4. **Strict Testing Policy**: Unit tests are absolutely mandatory for any logic changes. Manual testing is strictly forbidden.
5. **Error/Doubt Protocol**: If an error occurs, if code is questionable, or if architectural changes are deemed necessary, the AI must NOT automatically modify the code. It must notify the user and await explicit guidance.
6. **Execution Confirmation**: The AI must receive explicit user authorization before transitioning from Planning to modifying files in Execution.
...
8. **Phase File Immutability**: The AI Agent is strictly forbidden from modifying, overwriting, or deleting any historical `task`, `Implementation`, or `Walkthrough` files from previous phases. Any NEW development or follow-up work MUST be tracked in a new phase with a higher number.
