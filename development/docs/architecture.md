...
graph TD
    ...
    MainApp --> OpsPilot[/agent Sub-app]
    Routers --> Services[Backend Services]
    AIAgent --> AgentRouter[AI Agent Router]
    OpsPilot --> OpsRouter[Standalone Agent Router]
...
- `api/index.py`: repository-root adapter used by Vercel. It inserts `development` into `sys.path` and imports `web.backend.app.main:app`.
...
- `agent/ui/backend/main.py`: standalone sub-application mounted by the main app at `/agent`.
...
- Render starts `uvicorn web.backend.app.main:app` from `development`.
- The main app serves `/static` for the shared frontend and mounts `/ai-agent` for the AI assistant.
- The main app also mounts `/agent` for the standalone lead-focused agent surface.
...
- The standalone agent exposes dedicated routes through `agent/ui/backend/router.py`.
